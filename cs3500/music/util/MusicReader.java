  ...
  public static <T> T parseFile(Readable readable, CompositionBuilder<T> piece) {
    ...
    while (scanner.hasNext()) {
      ...
      switch (lineType) {
        ...
        case "repeat":
          try {
            piece.setRepeat1(scanner.nextInt());
          } catch (NoSuchElementException e) {
            throw new IllegalArgumentException(
                    "Malformed repeat1 line: " + scanner.nextLine());
          }
          break;
        case "repeat-bound":
          try {
            piece.setRepeat2(scanner.nextInt(), scanner.nextInt(), scanner.nextInt());
          } catch (NoSuchElementException e) {
            throw new IllegalArgumentException(
                    "Malformed repeat2 line: " + scanner.nextLine());
          }
          break;
        case "alt-end":
          List<Integer> altend = new ArrayList<>();
          try {
            while (scanner.hasNextInt()) {
              altend.add(scanner.nextInt());
              altend.add(scanner.nextInt());
            }
            piece.setAlternateEndings(altend);
          } catch (NoSuchElementException e) {
            throw new IllegalArgumentException(
                    "Malformed Alternate Ending line: " + scanner.nextLine());
          }
          break;
        case "note":
          try {
